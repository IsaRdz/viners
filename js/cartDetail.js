const renderDetail = (wine) => {
  return `<div id="Container${wine.id}" class="item-cart">
                ...
                <form class="count-product">
                    <button id="${
                      wine.id
                    }" type="button" class="button-item-cart subtract" aria-label="Subtract one item">-</button>
                    <span id="${wine.id}" class="show-units">${
    wine.cantidad
  }</span>
                    <button id="${
                      wine.id
                    }" type="button" class="button-item-cart add" aria-label="Add one item">+</button>
                </form>
                <div class="item-price">
                    <p id="Precio${wine.id}">$ ${
    wine.precio * wine.cantidad
  }</p>
                </div>
...
const cleanProductListener = () => {
  ...
  clenaProductButtons.forEach((clenaProductButton) => {
    clenaProductButton.addEventListener("click", (cleanProduct) => {
      var CartProducts = JSON.parse(localStorage.getItem("CartProducts"))
      ProductEliminated = CartProducts.filter(
        (product) => product.id == cleanProduct.target.id
      )
      for (let i = 0; i < ProductEliminated[0].cantidad; i++) {
        document.querySelector(".cart_products_total").innerHTML--
      }
      let CartProductsFiltered = CartProducts.filter(
        (product) => product.id != cleanProduct.target.id
      )
          ...
          unitsContainer.forEach((Unit) => {
            if (Unit.id == event.target.id) {
              if (Unit.innerHTML > 1) {
                ...
                document.querySelector(`#Precio${event.target.id}`).innerHTML =
                  CartProduct.cantidad * CartProduct.precio
...
const renderTotalPrice = () => {
  ...
  let preciototal = CartProducts.reduce(
    (accumulator, product) => accumulator + product.cantidad * product.precio,
    0
  )
